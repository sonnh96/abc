...
email status until they click the link in the confirmation email.`
    ...
    loginAttempts: {
      type: 'number',
      defaultsTo: 0,
      description: 'Wrong password login attempts',
      example: 0
    },
    captchaVerified: {
      type: 'boolean',
  ...
  increaseAttempt: async function (emailAddress) {
    const user = await User.findOne({emailAddress: emailAddress});
    const newAttempts = user.loginAttempts + 1;
    await User.update({emailAddress: emailAddress}, {loginAttempts: newAttempts});
    return newAttempts;
  }
};
