...
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-`);
      ...
      const mailerConfig = sails.config.custom.mailerConfig;
      if (mailerConfig) {
        const mailer = sails.helpers.nodejsMailer.with({
          transporter: mailerConfig.transporter,
          from: sails.config.custom.fromEmailAddress || 'noreply@example.com',
        });
        await mailer.send({
