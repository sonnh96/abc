...
parasails.registerComponent('modal', {
  ...
  //  ╠═╣ ║ ║║║║
  //  ╩ ╩ ╩ ╩ ╩╩═╝
  template: `<transition name="modal" v-on:leave="leave" v-bind:css="false">
  <div class="modal fade" tabindex="-1" role="dialog" >
    <div class="modal-dialog custom-width" role="document" :class="customClass">
      <div class="modal-content">
        <div class="modal-header">
          <slot name="title">
            <h5 class="modal-title">Modal title</h5>
          </slot>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close" ref="close_btn">
  ...
  mounted: function () {
    ...
    $(this.$el).modal({
      ...
      // when clicked.
      backdrop: this.closeClickOutside ? true : 'static',
      show: true
    });
