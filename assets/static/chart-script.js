...
var chartDrawer = (function ($, d3, _) {
  ...
  function loadChart(chatLoadOptions) {
    ...
    d3.csv(chatLoadOptions.url, type, function (err, data) {
      ...
      var xRange = d3.extent(data.map(function (d, i) {
        return i;
      }));

      function dateBefore(index) {
        return data[Math.ceil(index)].date;
      }

      function dateAfter(index) {
        return data[Math.min(data.length - 1, Math.ceil(index))].date;
      }

      function dateNearest(index) {
        return data[Math.min(data.length - 1, Math.round(index))].date;
      }

      var dateXRange = [dateBefore(xRange[0]), dateAfter(xRange[1])];
      ...
      var xAxis = d3.svg.axis().scale(x).tickFormat(function (d) {
          return legendFormat(new Date(dateNearest(d)));
        }).orient('bottom'),
        xAxis2 = d3.svg.axis().scale(x2).tickFormat(function (d) {
          return legendFormat(new Date(dateNearest(d)));
        }).orient('bottom'),
        yAxis = d3.svg.axis().scale(y).orient('left');

      var range = legend.append('text')
        .text(legendFormat(new Date(dateXRange[0])) + ' - ' + legendFormat(new Date(dateXRange[1])))
      ...
      function mousemove() {
        ...
        var x0Date = dateNearest(x0);
        var i = bisectDate(data, x0Date, 1);
        var d0 = data[i - 1];
        var d1 = data[i];
        var d = x0Date - d0.date > d1.date - x0Date ? d1 : d0;
        var info = legendFormat(new Date(d.date)) + ' - '
          + localizedLabels.close + ': ' + d.price + ' '
          + localizedLabels.vwap + ': ' + d.average;
        helperText.text(info);
        priceTooltip.attr('transform', 'translate(' + x(x0) + ',' + y(d.price) + ')');
        averageTooltip.attr('transform', 'translate(' + x(x0) + ',' + y(d.average) + ')');
      }

      function brushed() {
        var ext = brush.extent();

        var extDate = [dateBefore(ext[0]), dateAfter(ext[1])];
        if (!brush.empty()) {
          x.domain(brush.empty() ? x2.domain() : brush.extent());
          y.domain([
            d3.min(data.map(function (d, i) {
              return (i >= ext[0] && i <= ext[1]) ? d.price : max;
            })),
            d3.max(data.map(function (d, i) {
              return (i >= ext[0] && i <= ext[1]) ? d.price : min;
            }))
          ]);
          range.text(legendFormat(new Date(extDate[0])) + ' - ' + legendFormat(new Date(extDate[1])));
          focusGraph.attr('x', function (d, i) {
            return x(i);
          });
          var days = Math.ceil((extDate[1] - extDate[0]) / (24 * 3600 * 1000));
